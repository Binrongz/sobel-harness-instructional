...
sobel_filtered_pixel(float *s, int i, int j , int ncols, int nrows, float *gx, float *gy)
{
   ...
   for (int di = -1; di <= 1; di++) {
      for (int dj = -1; dj <= 1; dj++) {
         ...
         if (ni >= 0 && ni < nrows && nj >= 0 && nj < ncols) {
            ...
            // Calculate the index of the filter weights 
            // (di+1) and (dj+1) convert [− 1,0,1] to [0,1,2] 
            // Then convert to 1D index of 3x3 array 
   ...
   t = sqrt(Gx_sum * Gx_sum + Gy_sum * Gy_sum);
...
do_sobel_filtering(float *in, float *out, int ncols, int nrows)
{
   ...
   // collapse(2) combines two nested loops into a single parallel region 
   #pragma omp parallel for collapse(2) 
   for (int i = 0; i < nrows; i++) { 
      for (int j = 0; j < ncols; j++) { 
      // Calculate the index of output array 
      int index = i * ncols + j; 
      
      // Call sobel_filtered_pixel to calculate the value at this location 
      out[index] = sobel_filtered_pixel(in, i, j, ncols, nrows, Gx, Gy);
