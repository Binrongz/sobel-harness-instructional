...
sobel_filtered_pixel(float *s, int i, int j , int ncols, int nrows, float *gx, float *gy)
{
   ...
   for (int di = -1; di <= 1; di++) {
      for (int dj = -1; dj <= 1; dj++) {
         // Calculate the actual coordinates of neighbor pixels
         int ni = i + di;  // neighbor i
         int nj = j + dj;  // neighbor j

         // Boundary checking
         if (ni >= 0 && ni < nrows && nj >= 0 && nj < ncols) {
            // Computes the index in a 1D array
            int pixel_index = ni * ncols + nj;

            // Compute the filter index
            int filter_index = (di + 1) * 3 + (dj + 1); 
            
            // Accumulate the convolution results 
            Gx_sum += s[pixel_index] * gx[filter_index]; 
            Gy_sum += s[pixel_index] * gy[filter_index];
         }
      }
   }
   t = sqrtf(Gx_sum * Gx_sum + Gy_sum * Gy_sum);
      ...
      float *gx, float *gy) // gx and gy are stencil weights for the sobel filter
{
   ...
   // index = start of current block + intra-block thread offset
   int index = blockIdx.x * blockDim.x + threadIdx.x;

   // Calculate step size (total number of threads)
   int stride = blockDim.x * gridDim.x;

   // Total pixels
   int total_pixels = ncols * nrows;

   // Process multiple pixels with stride loop
   // Each thread may process multiple pixels
   for (int idx = index; idx < total_pixels; idx += stride) {
      // Convert from 1D index to 2D coordinates (i, j)
      int row = idx / ncols;   // Line number
      int col = idx % ncols;   // Column number

      // Call sobel_filtered_pixel to calculate the value at this location
      if (row < nrows && col < ncols) {
            d[idx] = sobel_filtered_pixel(s, row, col, ncols, nrows, gx, gy);
         }
   }
...
#define ONE_OVER_255 0.003921568627451
   ...
   if (ac == 3) {
      ...
      fprintf(stderr, "Usage: %s [num_blocks num_threads_per_block]\n", av[0]);
      return 1;
   }

   if (nBlocks <= 0 || nThreadsPerBlock <= 0) {
      fprintf(stderr, "Error: invalid block/thread configuration.\n");
      return 1;
   }
